...
#                    #    })
    ...
    def _as_ics(self):
        if not self.initialized:
            self._lazy_init()
        talks = [get_template('cfp/ics/talk.ics').render({'site': self.site, 'talk': talk}) for talk in self.talks]
        return get_template('cfp/ics/main.ics').render({'site': self.site, 'talks': '\r\n'.join(talks)})
